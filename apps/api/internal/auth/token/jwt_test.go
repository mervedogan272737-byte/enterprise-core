...
func TestValidateAccessToken_ExpiredToken(t *testing.T) {
	...
	if err != nil {
		t.Fatalf(
			"failed to generate expired token: %v",
			err,
		)
	}
...
func TestValidateAccessToken_WrongSecret(t *testing.T) {
	...
	wrongManager := NewManager(
		"wrong-secret",
		time.Hour,
	)

	_, err = wrongManager.ValidateAccessToken(
		tokenString,
	)

	if err != ErrInvalidToken {
		t.Fatalf(
			"expected ErrInvalidToken for wrong secret, got %v",
			err,
		)
	}
}
...
func TestValidateAccessToken_RejectsDifferentSigningMethod(t *testing.T) {
	...
	if err != ErrInvalidToken {
		t.Fatalf(
			"expected ErrInvalidToken for different signing method, got %v",
			err,
		)
	}
}
